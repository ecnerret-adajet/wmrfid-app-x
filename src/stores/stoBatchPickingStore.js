...
export const useStoBatchPickingStore = defineStore('batches', () => {
    const batchList = ref([]);
    ...
    const selectedBatchPalletCount = ref(null)
    ...
    async function fetchOpenQuantity(params) {
        ...
        try {
            ...
            this.stoDetails.open_quantity = data;
            return data;
        } catch (error) {
    ...
    async function fetchAvailableCommodities(params) {
        ...
        try {
            const { data } = await axios.post(`transfer-orders/get-available-commodities-sap`, {
            ...
            let remainingRequiredQty = parseInt(params.po_quantity) || 0;
    ...
    const fetchHeaderDetails = async (params) => {
        try {
            ...
            const response = await axios.get(`batch-picking/get-sto-details`, { params });
            if (response.data.success) {
                stoDetails.value = response.data.data;
            } else {
