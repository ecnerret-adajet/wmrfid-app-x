    ...
    if (data.results && Array.isArray(data.results)) {
      ...
      if (failedChunks.length > 0) {
        ...
        if (method === 'simulate') {
          ...
          dialogAlert.value = { show: true, type: 'warning', message: `QC Disposition approval completed with errors. ${totalSuccess} of ${data.total_items} items processed.` }
        }
        return
      }
      ...
      if (method === 'simulate') {
        ...
        dialogAlert.value = { show: true, type: 'success', message: `QC Disposition approved successfully! ${data.total_items} items processed across ${data.total_chunks} chunk(s).${docInfo}` }
        simulateCompleted.value = false
