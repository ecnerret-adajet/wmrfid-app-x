...
const plantCode = authStore.user?.assigned_plant?.plant_code;
const sloc = authStore.user?.assigned_plant?.default_storage_location?.code;
...
const loadItems = ({ page: pageVal, itemsPerPage: perPage, sortBy }) => {
    ...
    ApiService.query(`quality-control/goods-movement-logs/${plantCode}/${sloc}`, {
        params: {
            page: pageVal,
