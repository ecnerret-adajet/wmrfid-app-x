<template>
    <v-container fluid>
        ...
        <Loader :show="isLoading || qualityControlStore.loading" />
...
function syncTransferRequests() {
    ...
    if (plant_code && sloc && forklift) {
        ...
        qualityControlStore.fetchTransferRequests(
            plant_code,
            sloc,
            forklift,
            {
...
const fetchWithSearch = lodashDebounce(() => {
    ...
    if (plant_code && sloc && forklift) {
        ...
        if (selectedStatus.value === 'Pending' || selectedStatus.value === 'Invalid Request') {
            ...
            qualityControlStore.fetchTransferOrders(
                plant_code,
                sloc,
                forklift,
                {
...
async function generateTransferOrder(item) {
    ...
    try {
        const result = await qualityControlStore.generateTransferOrder(plant_code, sloc, forklift, item.id);
        // Wait for the store to refresh
        await qualityControlStore.fetchTransferRequests(plant_code, sloc, forklift);
        // Find the updated item
        const updated = qualityControlStore.items?.transfer_requests?.find(tr => tr.id === item.id);
        if (updated) {
...
const confirmWrapping = async (text) => {
    ...
    try {
        const response = await qualityControlStore.confirmWrapping({
        ...
        qualityControlStore.fetchTransferRequests(route.params.plant_code, route.params.sloc, route.params.forklift);
    }
}
...
const confirmPutaway = async (text) => {
    ...
    isLoading.value = true;
    try {
        const response = await qualityControlStore.confirmPutaway({
            plant_code: route.params.plant_code,
            sloc: route.params.sloc,
            forklift: route.params.forklift,
            physical_id: selectedTransferRequest.value.physical_id,
