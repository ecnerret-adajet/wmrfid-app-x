...
onMounted(async () => {
    ...
    const po_number = route.query.po_number;
    const po_item = route.query.po_item;

    try {
        const response = await axios.get(`transfer-orders/check-auth`, {
            params: {
                email,
                signature,
                expires,
        ...
        store.verifyAuth();
        router.push({ name: 'sto-batch-picking', params: { po_number: po_number, po_item: po_item } });
    } catch (error) {
        console.log(error);
