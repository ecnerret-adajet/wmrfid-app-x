...
const loadItems = ({ page, itemsPerPage, sortBy, search }) => {
    ...
    ApiService.query('datatable/permissions',{
        params: {
            page,
            itemsPerPage,
...
const handleDelete = async () => {
    ...
    try {
        const response = await ApiService.delete(`permissions/${selectedPermission.value.id}/delete`)
        ...
        toast.value.message = 'Permission deleted successfully!'
...
const handleUpdate = async () => {
    ...
    toast.value.show = false
    try {
        const response = await ApiService.put(`permissions/${selectedPermission.value.id}/update`, form.value)
        isLoading.value = false;
        ...
        toast.value.message = 'Permission updated successfully!'
        toast.value.show = true;
    } catch (error) {
...
<template>
    ...
    <DeleteModal @close="deleteDialog = false" :show="deleteDialog" dialog-title="Delete Permission">
        <template #default>
            <VCardText>
                <p class="text-h5 text-bold-emphasis ps-2">Are you sure you want to delete this permission? This action is irreversible</p>
            </VCardText>
    ...
    <EditingModal v-if="form.name && form.description" @close="editDialog = false" 
        :show="editDialog" :dialog-title="`Update ${selectedPermission.name}`">
        <template #default>
            <v-form @submit.prevent="handleUpdate">
                <v-text-field class="mt-6" density="compact" 
                    label="Name"
                    v-model="form.name" 
                    :rules="[value => !!value || 'Permission name is required']"
                />
                <v-text-field class="mt-6" density="compact" 
                    label="Description"
