...
const loadItems = ({ page, itemsPerPage, sortBy, search }) => {
    ...
    ApiService.query(`production-runs/get-available-rfid/${props.repacking?.to_material?.plant?.plant_code}`, {
        params: {
            page,
            itemsPerPage,
...
watch(selectedTagType, async (newVal) => {
    ...
    try {
        const response = await ApiService.query(`production-runs/get-available-rfid/${props.repacking?.to_material?.plant?.plant_code}`, {
            params: {
                page: page.value,
...
const assignConfirm = async () => {
    ...
    try {
        const response = await ApiService.post('repackings/assign-to-pallets', form)
...
const handleAssign = () => {
    const invalid = selectedItems.value.some(item => !item.quantity || Number(item.quantity) === 0);

    if (invalid) {
        toast.value.color = 'error';
        toast.value.message = 'Please enter a valid quantity for all selected items.';
        toast.value.show = true;
        return;
    }
...
<template>
                    ...
                    <div>
                        ...
                        <VDataTableServer v-model:items-per-page="itemsPerPage" v-model="selectedItems"
                            ...
                            <template #item.quantity="{ item }">
                                <div v-if="selectedItems.some(selected => selected.id === item.id)">
                                    <v-text-field v-model="item.quantity" density="compact" hide-details
                                        placeholder="QTY" :rules="[v => !!v || 'Quantity is required']" />
                                </div>
                            </template>
    ...
    <v-dialog v-model="showAssignConfirm" max-width="600px" persistent>
        <v-sheet class="px-4 pt-8 pb-4 text-center mx-auto" elevation="12" max-width="600" rounded="lg" width="100%">
            ...
            <h2 class="text-h4 mb-6">Assign batch <strong>{{ repacking?.batch }}</strong> to the selected
                pallets?
            </h2>
