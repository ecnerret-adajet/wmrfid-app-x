...
const headers = [
    ...
    {
        title: 'GOODS ISSUE STATUS',
        key: 'goods_issue_status',
    },
    {
        ...
        try {
            ...
            if (showStocks.value === true) {
                const delParams = {
                    ...
                    material_code: selectedDeliveryItem.value.material_number,
                    delivery_document: deliveryData.value.delivery_document,
                    item_number: selectedDeliveryItem.value.item_number,
                    delivery_quantity: selectedDeliveryItem.value.delivery_quantity,
                    sales_unit: selectedDeliveryItem.value.sales_unit,
                    storage_location: selectedDeliveryItem.value.storage_location,
                    plant: selectedDeliveryItem.value.plant,
...
const submitProposal = async () => {
    ...
    formData.append('material_name', selectedDeliveryItem.value.material_description);
    formData.append('material_code', parseInt(selectedDeliveryItem.value.material_number));
    ...
    formData.append('delivery_quantity', selectedDeliveryItem.value.delivery_quantity);
    ...
    formData.append('plant', selectedDeliveryItem.value.plant);
    formData.append('sloc', selectedDeliveryItem.value.storage_location);
...
<template>
    ...
    <template #item.goods_issue_status="{ item }">
        {{ item.goods_issue_status }}
    </template>

    <template #item.delivery_items="{ item }">
        {{ item.delivery_items.length }}
    </template>
            ...
            <v-card-text>
                ...
                <v-card-text v-if="!showStocks">
                    ...
                    <v-table class="mt-4 striped">
                        ...
                        <tbody>
                            <tr v-for="(item, index) in deliveryData?.delivery_items" :key="index">
                                ...
                                <td class="text-center">
                                    ...
                                    <v-badge v-if="!item.delivery_reserved_orders || item.delivery_reserved_orders.length === 0"
                                        color="warning"
                                        content="No Pallet"
                                    ...
                                    <v-badge v-else-if="item.delivery_reserved_orders.length > 0 && (parseInt(item.total_reserved_pallets) === parseInt(item.delivery_quantity))"
                                        color="success"
                                        content="Reserved"
                                        class="text-uppercase"
                                        inline
                                    ...
                                    <div v-else class="d-flex flex-column py-3">
                                        <v-badge
                                            ...
                                            inline
                                        ></v-badge>
                                        <span class="mt-1">{{ item.total_reserved_pallets }} out of {{ item.delivery_quantity }} {{ item.sales_unit }}(S)</span>
                                    </div>
                                </td>
                                <td class="text-center">
                                    <v-btn
                ...
                <v-card-text v-else>
                    <v-skeleton-loader v-if="deliveryOrder.filter.loading_available_stocks" type="article"></v-skeleton-loader>
                    <div v-else>
                        <div v-if="parseInt(selectedDeliveryItem.delivery_quantity) !== parseInt(selectedDeliveryItem.total_reserved_pallets)">
                    ...
                    <div v-else class="d-flex justify-end mt-4 py-4">
                        <v-btn variant="outlined" color="grey" class="mr-2" @click="backToDeliveryItems">Back To Delivery Items</v-btn>
                        ...
                        <v-btn color="success" type="submit"
                            v-if="parseInt(selectedDeliveryItem.delivery_quantity) !== parseInt(selectedDeliveryItem.total_reserved_pallets)"
        ...
        <v-card class="pa-4">
            <div class="text-center">
                ...
                <p class="font-weight-medium text-medium-emphasis">
                    ...
                    initially held for {{ selectedDeliveryItem.delivery_quantity }} required {{ selectedDeliveryItem.sales_unit }}(s).
                </p>
