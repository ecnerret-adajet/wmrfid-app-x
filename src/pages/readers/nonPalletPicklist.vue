...
const fetchData = async () => {
    loading.value = true;
    try {
        const response = await ApiService.post(`non-pallet-picklist-tapping/${readerId}/${bay}`);
...
const sapLoadEnd = async (shipmentNumber) => {
    ApiService.get(`non-pallet-picklist/load-end/${shipmentNumber}`)
        .then(response => {
            window.location.reload();
        })
};
...
const fetchShipmentDetails = async (shipmentNumber) => {
    try {
        ...
        let url = `non-pallet-picklist/shipment-picklist/${shipmentNumber}`;
            ...
            <v-card v-if="shipmentData.shipment.wm_load_start_date && shipmentData.shipment.wm_load_end_date === null"
                ...
                <VRow class="w-100" align="center">
                    <VCol cols="12" md="8" class="d-flex flex-column justify-center">
                        ...
                        <v-progress-linear color="warning" height="14" rounded indeterminate>
                        </v-progress-linear>
                    </VCol>
            ...
            <v-card v-else class="mb-4 pa-4 d-flex align-center" color="primary" variant="tonal" elevation="3"
                ...
                <VRow class="w-100" align="center">
                    <VCol cols="12" md="8" class="d-flex flex-column justify-center">
                        ...
                        <v-progress-linear :model-value="(refreshTimer / 120) * 100" color="primary-light" height="8"
                            rounded class="mt-2" />
                    </VCol>
                    <VCol cols="12" md="4" class="d-flex flex-column align-center justify-center">
                                    ...
                                    <v-sheet height="100%">
                                        <div v-for="(delivery, index) in chunk" :key="index">
                                            <VRow no-gutters style="border: 1px solid #1ab394;">
                                                <VCol md="3"
                                                    class="px-3 py-2 text-center rightBorderedGreen d-flex align-center justify-center"
                                                ...
                                                <VCol md="3"
                                                    class="px-3 py-1.5 text-center rightBorderedGreen d-flex align-center justify-center"
                                                ...
                                                <VCol md="3"
                                                    class="px-3 text-center rightBorderedGreen d-flex align-center justify-center"
