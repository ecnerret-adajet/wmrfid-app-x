...
const setTimeInSeconds = ref(300);
...
onMounted(() => {
    ...
    const channelNameBase = `sto.${readerId}.${bay}`;

    picklistLogsChannel = echo.channel(`${channelNameBase}.sto-picklist-logs`);
    picklistRefreshChannel = echo.channel(`${channelNameBase}.sto-picklist-refresh`);
    driverTapOutChannel = echo.channel(`${channelNameBase}.sto-driver-tap-out`);

    picklistLogsChannel.listen('StoPicklistLogsEvent', onPicklistLogsEvent);
    picklistRefreshChannel.listen('StoPicklistRefreshEvent', onPicklistRefreshEvent);
    driverTapOutChannel.listen('StoDriverTapOutEvent', onDriverTapOutEvent);
});
...
const onPicklistLogsEvent = (data) => {
    console.log(data)
    if (data.picklistLog?.current_shipment_number == stoData.transport?.shipment) {
        ...
        if (batch && (is_loaded == false || is_loaded == 0)) {
            ...
            const delivery = stoData.items.find(d => String(d.batch).trim() === normalizedBatch);
...
const reloadPageChecker = () => {
    setInterval(function () {
        const isLoadingInProgress = stoData.transport.wm_load_start_date && stoData.transport.wm_load_end_date === null;
        const isWaitingForBatches = stoData.transport?.no_batches_yet === true;
        const isNoInventory = stoData.transport?.batch_no_inventory_yet === true && stoData.transport?.no_batches_yet === false;
    ...
    try {
        ...
        if (response.data.original?.error) {
            ...
            if (response.data.shipment_number === 'NO SHIPMENT') {
                ...
                dialogVisible.value = true;
            } else if (response.data.shipment_number === 'TRANSFER') {
                await fetchTransportDetails(response.data.transport_number)
            } else {
                // If shipment found, redirect back to shipment picklist screen
                handleTransferRedirection()
            }
        }
    } catch (error) {
        dialogVisible.value = true
...
const isShipmentDetected = ref(false);
const handleTransferRedirection = () => {
    isShipmentDetected.value = true;
    errorMessage.value = 'Shipment Detected. Redirecting Page...';

    setTimeout(() => {
        router.push({
            name: 'shipment-picklist',
            params: { reader: readerId, bay: bay }
        }).catch(() => {
        });
    }, 2500);
};
...
const fetchTransportDetails = async (transportNumber) => {
    try {
        ...
        let url = `picklist/sto-picklist/${transportNumber}`;
        ...
        if (response.data.result == 'S') {
            stoData.items = response.data.picklist;
            stoData.transport = response.data;
            console.log(stoData.transport)
            // totalRead.value = stoData.transport?.total_pallet_to_load;
        } else {
            // stoData.transport = response.data;
...
const stoChunks = computed(() => chunkArray(stoData.items || [], 5))
...
const totalLoadedQty = computed(() =>
    stoData.items?.reduce((sum, item) => sum + (item.loaded_qty || 0), 0)
);

const totalRequiredQtyByBatch = computed(() =>
    stoData.items?.reduce((sum, item) => sum + (Number(item.required_pallets) || 0), 0)
);

const isLoadedQtySatisfiedPerBatch = computed(() => {
    if (!stoData.items?.length) return false;

    return stoData.items.every(item => {
...
const onDriverTapOutEvent = (data) => {
    ...
    // if (totalLoadedQty.value > 0 && totalLoadedQty.value >= stoData.transport?.total_pallet_to_load) {
    console.log(data);
    if (data.driverTapOut?.shipment_number == stoData.transport?.shipment && data.driverTapOut?.load_end_date === null) {
...
watch(
    ...
    ([isSatisfiedPerBatch, tappingLoadEndFound]) => {
        if (tappingLoadEndFound === true && isSatisfiedPerBatch) {
            sapLoadEnd(stoData.transport.shipment);
        }
    },
    { immediate: true }
);
...
const carouselPageIndicator = computed(() => {
    const totalPages = stoChunks.value.length || 0;
...
const goToPrevCarouselPage = () => {
    const totalPages = stoChunks.value.length;
...
<template>
    ...
    <div v-else class="py-2 px-8 whiteBackground">
        ...
        <div>
            <v-card
                v-if="stoData.transport.wm_load_start_date && stoData.transport.wm_load_end_date === null && !isLoadedQtySatisfiedPerBatch"
            ...
            <v-card
                v-else-if="stoData.transport.wm_load_end_date === null && is_tapping_load_end_found === false && isLoadedQtySatisfiedPerBatch"
            ...
            <VRow>
                <VCol md="8">
                    <VList lines="one" density="compact"
                        style="border: 2px solid #1b6be2; padding-top: 0px !important;">
                        <VListItem>
                                    ...
                                    <VRow class="table-row">
                                        <VCol md="6" class="d-inline-flex align-center">
                                            ...
                                            <span class="text-h6 text-uppercase ml-3 font-weight-black "
                                                style="margin-top: 1px;">Transport No.</span>
                                        </VCol>
                                        <VCol md="6" class="d-inline-flex align-center">
                                            <span class="font-weight-bold">{{ stoData?.transport?.transport_number
                                                }}</span>
                                        </VCol>
                                    </VRow>
                                </VCol>
                                <VCol md="6" class="table-cell d-inline-flex">
                                    <VRow class="table-row">
                                        <VCol md="6" class="d-inline-flex align-center">
                                            ...
                                            <span class="text-h6 text-uppercase ml-3 font-weight-black "
                                                style="margin-top: 1px;">Receiving Plant & Sloc</span>
                                        </VCol>
                                        <VCol md="6" class="d-inline-flex align-center">
                                            <span class="font-weight-bold">2119 - W107</span>
                                        </VCol>
                                    </VRow>
                                </VCol>
                            </VRow>
                        </VListItem>
                        {{ stoData?.transport?.transport_tracking }}
                        <VListItem>
                            <VRow class="table-row" no-gutters>
                                <VCol md="6" class="table-cell d-inline-flex">
                                    <VRow class="table-row">
                                        ...
                                        <VCol md="6" class="d-inline-flex align-center">
                                            <div class="font-weight-bold">{{
                                                stoData?.transport?.transport?.transport_tracking?.vendor_name }}
                                            </div>
                                        </VCol>
                                    </VRow>
                                </VCol>
                                <VCol md="6" class="table-cell d-inline-flex">
                                    <VRow class="table-row">
                                        ...
                                        <VCol md="6" class="d-inline-flex align-center">
                                            <div class="font-weight-bold">{{
                                                stoData?.transport?.transport?.transport_tracking?.vendor_code }}
                                            </div>
                                        </VCol>
                                    </VRow>
                                </VCol>
                            ...
                            <VRow class="table-row" no-gutters>
                                    ...
                                    <VRow class="table-row">
                                        ...
                                        <VCol md="6" class="d-inline-flex align-center">
                                            <span class="font-weight-bold">{{
                                                stoData?.transport?.transport?.vehicle?.plate_number
                                            }}</span>
                                        </VCol>
                                    </VRow>
                                </VCol>
                                <VCol md="6" class="table-cell d-inline-flex">
                                    <VRow class="table-row">
                                        ...
                                        <VCol md="6" class="d-inline-flex align-center">
                                            <span class="font-weight-bold">{{
                                                formatDateTime(stoData.transport.wm_load_start_date,
                                                    stoData.transport.wm_load_start_time) }}</span>
                                        </VCol>
                                    </VRow>
                            ...
                            <VRow class="table-row" no-gutters>
                                <VCol md="6" class="table-cell d-inline-flex">
                                    <VRow class="table-row">
                                        ...
                                        <VCol md="6" class="d-inline-flex align-center">
                                            <span class="font-weight-bold">{{
                                                stoData.transport?.transport?.driver?.full_name
                                                }}</span>
                                        </VCol>
                                    </VRow>
                                </VCol>
                                <VCol md="6" class="table-cell d-inline-flex">
                                    <VRow class="table-row">
                                        ...
                                        <VCol md="6" class="d-inline-flex align-center">
                                            <span class="font-weight-bold">{{
                                                formatDateTime(stoData.transport.wm_load_end_date,
                                                    stoData.transport.wm_load_end_time) }}</span>
                                        </VCol>
                                    </VRow>
                                </VCol>
                        ...
                        <!-- <VListItem>
                            <VRow class="table-row" no-gutters>
                                <VCol md="6" class="table-cell d-inline-flex">
                                    <VRow class="table-row">
                                        <VCol md="6" class="d-inline-flex align-center">
                                            <v-icon icon="ri-box-2-line" style="width: 32px; height: 32px"
                                                class="icon-class" color="primary"></v-icon>
                                            <span class="text-h6 text-uppercase ml-3 font-weight-black "
                                                style="margin-top: 1px;">PO
                                                Number</span>
                                        </VCol>
                                        <VCol md="6" class="d-inline-flex align-center">
                                ...
                                <VCol md="6" class="table-cell d-inline-flex">
                                    <VRow class="table-row">
                                        <VCol md="6" class="d-inline-flex align-center">
                                            <v-icon icon="ri-file-list-line" style="width: 32px; height: 32px"
                                                class="icon-class" color="primary"></v-icon>
                                            <span class="text-h6 text-uppercase ml-3 font-weight-black "
                                                style="margin-top: 1px;">PO
                                                Item</span>
                ...
                <VCol md="4">
                    <v-sheet class="mx-auto px-6 py-1" elevation="2"
                        style="height: 100%; display: flex; flex-direction: column; background-color: #1b6be2;">
                        ...
                        <div>
                            <h2 class="text-h4 font-weight-black mt-8" style="font-size: 5rem !important; color: #fff;">
                                <span>{{ totalLoadedQty }}</span> / {{ stoData.transport?.total_pallets_to_load }}
                            </h2>
                        </div>
            ...
            <div v-else class="mt-4">
                <div>
                    <VRow no-gutters>
                        <VCol md="3" style="font-size: 18px; background-color: #1b6be2;"
                        ...
                        <VCol md="3" class="text-uppercase px-3 py-2 text-center font-weight-black text-grey-100"
                            style="background-color: #1b6be2; font-size: 18px; border-left: 1px solid #fff; border-right: 1px solid #fff;">
                            Expected (SAP)
                        </VCol>
                        <VCol md="3" class="text-uppercase px-3 py-2 text-center font-weight-black text-grey-100"
                            style="background-color: #1b6be2; font-size: 18px; border-right: 1px solid #fff;">
                            Expected (WMRFID)
                        </VCol>
                        <VCol md="3" style="font-size: 18px; background-color: #1b6be2;"
                            class="text-uppercase px-3 py-2 text-center font-weight-black text-grey-100">
                    ...
                    <div>
                        <div>
                            <div v-if="stoData.items.length === 0" style="min-height: 100px;"
                            ...
                            <v-carousel v-else height="450" hide-delimiters :show-arrows="false" v-model="carouselIndex"
                                :cycle="stoChunks.length > 1" :interval="10000">
                                <v-carousel-item v-for="(chunk, index) in stoChunks" :key="index">
                                    <v-sheet height="100%">
                                        <div v-for="(delivery, index) in chunk" :key="index">
                                            <VRow no-gutters style="border: 1px solid #1b6be2;">
                                                <VCol md="3" class="px-3 py-2 text-center rightBorderedGreen">
                                                    <div class="text-center">
                                                        ...
                                                        <div>
                                                            <span style="color: #1b6be2;"
                                                                class="text-uppercase text-h5 font-weight-black">
                                                                {{ delivery.material_code }} - {{ delivery.batch }}
                                                            </span>
                                                            <br>
                            ...
                            <div v-if="stoChunks.length > 1" class="d-flex align-center justify-center mt-3"
                                style="gap: 12px;">
...
.rightBorderedGreen {
    border-right: 1px solid rgba(2, 49, 179, 0.5) !important;
}
...
.v-table .v-table__wrapper>table>tbody>tr:not(:last-child)>th {
    border-bottom: 1px solid rgba(2, 49, 179, 0.5) !important;
}
